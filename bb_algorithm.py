...
class BB:
    ...
    def relaxed(self, model):
        ...
        if model.status == GRB.OPTIMAL:
            ...
            self.cnt_node += 1
            return self.objVal, self.cnt_node, model, self.sol  # (목적함수값, n번째 node, model, 해)
        else:
    ...
    def prouning(self, model, idx, isfraction):
        ...
        self.cnt_node += 1
        heapq.heappush(self.pq, (self.objVal, self.cnt_node, upper_model, self.sol))
        self.cnt_node += 1
        heapq.heappush(self.pq, (self.objVal, self.cnt_node, lower_model, self.sol))
    ...
    def solve(self):
        ...
        while self.pq:
            ...
            node = self.relaxed(heapq.heappop(self.pq)[2])
            if node == 'Cannot find feasible solution':
                continue
            if self.sense == 1:
                if node[0] > self.current_best:
                    continue
            if self.sense == -1:
                if node[0] < self.current_best:
                    continue
            for i in range(self.num_vars):
                idx, isfraction = i, node[3][i]
                if isfraction % 1 == 0:  # 만약 int라면
                    continue
                else:
                    self.prouning(node[2], idx, isfraction)
